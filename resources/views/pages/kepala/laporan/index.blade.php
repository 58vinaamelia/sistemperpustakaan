    ...
    <div class="bg-white p-4 rounded-xl shadow mb-6">
        <form class="flex flex-wrap gap-4 items-end" method="GET">
            <div>
                ...
                <input type="date" name="dari" value="{{ request('dari') }}" class="border rounded-lg px-3 py-2 text-sm w-full">
            </div>

            <div>
                <label class="text-sm text-gray-600">Sampai Tanggal</label>
                <input type="date" name="sampai" value="{{ request('sampai') }}" class="border rounded-lg px-3 py-2 text-sm w-full">
            </div>
        ...
        <table class="w-full text-sm text-left">
            ...
            <tbody class="text-gray-700">
                ...
                <tr class="border-t hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3">{{ $item->tanggal_pinjam ? date('d-m-Y', strtotime($item->tanggal_pinjam)) : '-' }}</td>
                    <td class="px-4 py-3">{{ $item->tanggal_kembali ? date('d-m-Y', strtotime($item->tanggal_kembali)) : '-' }}</td>
                    <td class="px-4 py-3">
